...
void* Mempool::MIN_SLOT_MANAGER::allocSlot(size_t nSize) {
	if (nSize == 0) return NULL;
	size_t nSlots = (nSize + MIN_SLOT_SIZE - 1) / MIN_SLOT_SIZE;
...
void Mempool::MIN_SLOT_MANAGER::freeSlot(void* p) {
	...
	size_t offset = pSlot - (MIN_SLOT*)this; // the slot offset
	size_t bit = offset & 0x0000001f; // offset % 32
	size_t byte = offset >> 5; // offset / 32
#ifdef DEBUG
	if (((bmIndex[byte] >> bit) & 0x01) == 0 || ((bmHeaderIndex[byte] >> bit) & 0x01) == 0) {
		std::cout << "Error: free a not aloocated slot" << std::endl;
	}
#endif
	size_t index = bmHeaderIndex[byte] >> bit;
...
inline Mempool::MIN_SLOT* Mempool::MIN_SLOT_MANAGER::matchIndex(int i, size_t nSlots) {
	// Example:
...
void Mempool::free(void* minSlot) {
	for (auto l : minSlotManagerList) {
		if (((ULONG_PTR)minSlot & ~(PAGE_SIZE - 1)) == (ULONG_PTR)l)//if (((unsigned int)minSlot & ~(PAGE_SIZE - 1)) == (unsigned int)l)//gxg	
...
void *Mempool::allocPages(size_t size) {
	static_assert(((((7 * PAGE_SIZE + PAGE_SIZE - 1) >> 12) << 12) & (PAGE_SIZE-1)) == 0, "error");
	size_t alligned = ((size + PAGE_SIZE - 1) >> 12) << 12;
